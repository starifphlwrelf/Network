...
chaincodeQuery() {
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    ...
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["GetAllPatients"]}' >&log.txt
    res=$?
...
if [ "$CC_SKIP_PACKAGE_INSTALL" = "true" ]; then
  ...
  # peer0 of committee
  installChaincode 0 committee
  # peer1 of committee
  installChaincode 1 committee
  # peer0 of voter
  installChaincode 0 voter
  # peer1 of voter
  installChaincode 1 voter
  # peer0 of adminops
  installChaincode 0 adminops
  # peer1 of adminops
  installChaincode 1 adminops
fi
...
if [ "$CHANNEL_NAME" = "votechannel" ]; then
  ...
  queryInstalled 0 committee

 ## approve the definition for config.orgs
  approveForMyOrg 0 committee
  approveForMyOrg 0 voter
  approveForMyOrg 0 adminops
  ## check whether the chaincode definition is ready to be committed
  ## expect all of them to have approved

  checkCommitReadiness 0 committee
  checkCommitReadiness 0 voter
  checkCommitReadiness 0 adminops


  ## commit the definition
  commitChaincodeDefinition committee voter adminops

  ## query to see that the definition committed successfully
  queryCommitted 0 committee
  queryCommitted 0 voter
  queryCommitted 0 adminops
  ## Invoke the chaincode - this does require that the chaincode have the 'initLedger'
  ## method defined
  if [ "$CC_INIT_FCN" = "NA" ]; then
    infoln "Chaincode initialization is not required"
  else
    chaincodeInvokeInit committee
  fi
else
