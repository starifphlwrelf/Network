...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating committee Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-committee.yaml --output="organizations"
    res=$?
    ...
    infoln "Creating voter Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-voter.yaml --output="organizations"
    res=$?
    
     if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    fi
    infoln "Creating adminops Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-adminops.yaml --output="organizations"
    res=$?
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/committee/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating orderer1 Identities"

    createorderer1

    infoln "Creating orderer2 Identities"

    createorderer2

    infoln "Creating orderer3 Identities"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    if [ "$OS_ARCH" == "darwin-amd64" ]; then
      ...
      rm -rf organizations/fabric-ca/committee
      rm -rf organizations/fabric-ca/voter
      rm -rf organizations/fabric-ca/adminops
      rm -rf organizations/fabric-ca/orderer1
      rm -rf organizations/fabric-ca/orderer2
      rm -rf organizations/fabric-ca/orderer3
      ...
      ## remove fabric ca artifacts
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/committee'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/voter'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/adminops'

      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orderer1'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orderer2'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orderer3'
      # remove channel and script artifacts
...
CC_COLL_CONFIG="../../chaincode/collections_config.json"
